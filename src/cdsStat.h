#ifndef __CDS_STAT_H__
#define __CDS_STAT_H__
...
public:
  ...
  cdsKey(int _scorebin, int _cpg, int _deg, int _pos, int _aa) {
    scorebin = _scorebin;
    cpg = (_cpg == -1) ? 255 : (uint8_t)_cpg;
    deg = (_deg == -1) ? 255 : (uint8_t)_deg;
    pos = (_pos == -1) ? 255 : (uint8_t)_pos;
    aa = (_aa == -1) ? 255 : (uint8_t)_aa;
  }

  bool operator< (const cdsKey& x) const {
...
class cdsStat {
 ...
 cdsStat() : nBase(0), nPass(0), nInds(0), sumCons(0), sqCons(0) {}
  void setN(int n) { 
    nInds = n; 
    etas.clear();
    etas.resize(8*n,0); 
  ...
  void addVariant(uint8_t altAllele, int altCount) {
    //notice("addVariant(%d,%d), ++etas[%d];",altAllele,altCount,(altAllele-1) * 2 * nInds + (altCount-1));
    if ( altCount > 0 ) {
...
const double cdsStat::NAN_DBL = sqrt(-1.);  // assign double NAN value
